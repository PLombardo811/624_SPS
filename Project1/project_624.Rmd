...
ts_plot(ts_S02,"Group S02",names(S02_df_train))
```
Va02 has a downside trend and ACF declines as lag increase also indicate that there is a trend in the series. The scalloped shape of ACF also indicate there is seasonality  So we try STL decomposition firt  
...
fit_var02 <- stl(ts_S02$a, t.window=7, s.window="periodic",
  robust=TRUE)

fit_var02 %>% forecast(method="naive",h=140) %>%
  autoplot() 

```
we first fit a STL decomposition model 
#### ets:
```{r}
(fit.ets <- ets(ts_S02$a))
checkresiduals(fit.ets)
fit.ets %>% forecast(h=50) %>%
  autoplot() 
```
the ETS model doesn't look very good becasue there the ACF showing that the residuals is not wite nocie and there is a huge spike in the frequency distribuction plot. In addition the variation of the prediction is getting bigger which doesn't match the trend of the series. So probably won't use the model  


From the ACF plot, we can see there is autocorrelation in the series so we can try a ARIMA model. since the the ACF of the series is groping slowing, we know that the series is not stationary. After taking one difference the ACF drop quickly to 0 and the box test also showing the the series is stationary after first difference. In addtiotn the PACF plot has a huge spike in lag 1 and not obverous spike in other lags. so an ARIMA(1,1,0) model may be a good fit.
...
acf()
ndiffs(ts_S02$a)
Box.test(diff(ts_S02$a), lag=7, type="Ljung-Box")
ggAcf(ts_S02$a,main="ACF")
ggPacf(ts_S02$a,main="PACF")
(fit <- Arima(ts_S02$a, order=c(1,1,0)))

(fit <- Arima(ts_S02$a, order=c(1,1,0), seasonal=c(1,0,1)))
checkresiduals(fit)
```
The ACF of the residuals looks similar as  the ETS model. but the frequency plot looks good on  which suggest that ARIMA(1,1,0) is better than ETS but there can be a better ARIMA model 

So we are going to find the best ARIMA model using auto arima
```{r}
(fit.arima <- auto.arima(ts_S02$a))
checkresiduals(fit.arima)
accuracy(fit.arima)
fit.arima %>% forecast(h=140)
```
The ACF of the residuals look much better now since ACF for almost all the lags are close to 0 and the frequency plot also showing that the  residuals is normally distribucted. In addtion the AIC of the auto arima model is also better than arima(1,1,0) model  
...
fit_var02 <- stl(ts_S02$b, t.window=7, s.window="periodic",
  robust=TRUE)

fit_var02 %>% forecast(method="naive",h=140) %>%
  autoplot() 
(fit.ets <- ets(ts_S02$b))
checkresiduals(fit.ets)
fit.ets %>% forecast(h=140) %>%
  autoplot() 
```

```{r}
(fit.arima <- auto.arima(ts_S02$b))
checkresiduals(fit.arima)
accuracy(fit.arima)
fit.arima %>% forecast(h=140) %>%
  autoplot() 
...
```
the residual plot shows that the model is a good fit since the ACF of residuals are close to 0 and residuals are normally distribucted 
